...
final_dataframe.index = pd.to_datetime(bars_df[bars_df['symbol'] == 'TSLA']['timestamp']).dt.date
...
def decide_buy_sell_hold(dataframe, threshold):
    ...
    days_above_threshold = (dataframe['TSLA'] > threshold).sum()
    # Calculate percentage change in stock price over the specified time frame (e.g., 13 days)
percentage_change = closing_price.pct_change(periods=7) * 252

# Define threshold values for percentage change
...
# Calculate percentage change in stock price over the specified time frame (e.g., 13 days)
closing_price = final_dataframe['BB']  # Replace 'META' with your stock symbol
